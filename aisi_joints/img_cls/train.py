...
class ModelCheckpointWithFreeze(tf.keras.callbacks.ModelCheckpoint):
    """A class wrapping the Keras ModelCheckpoint class, setting all layers to
    trainable=True before saving, and then re-setting.
...
def fit_model(
    ...
    image_eval = EvaluateImages(mw.model, val_data, img_writer, 10)
    ...
    mw.model.fit(
        train_data,
        batch_size=mw.config.batch_size,
        validation_data=val_data,
        use_multiprocessing=False,
        workers=mw.config.workers,
        epochs=epochs,
        callbacks=callbacks,
        class_weight=mw.config.class_weights,
    )
...
def train(config: Config, mode: str):
    ...
    train_data = JointsSequence(
        config.dataset, "train", *input_size, batch_size=config.batch_size
    )
    ...
    if mode in ("both", "transfer"):
        try:
            fit_model(
                mw,
                config.transfer_config.optimizer,
                train_data,
                val_data,
                config.transfer_config.epochs,
                "transfer",
                metrics=metrics,
            )
    ...
    if mode in ("both", "finetune"):
        ...
        try:
            fit_model(
                mw,
                config.finetune_config.optimizer,
                train_data,
                val_data,
                config.finetune_config.epochs,
                "finetune",
                metrics=metrics,
            )
        except KeyboardInterrupt:
