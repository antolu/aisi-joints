...
from .train import ModelCheckpointWithFreeze
...
def tune_hyperparams(dataset_csv: str, config: Config, mode: str = "both"):
    ...
    if mode in ("both", "finetune"):
        ...
        checkpoint_path = get_latest(config.checkpoint_dir, lambda o: o.endswith(".h5"))

        log.info("\n".join([separator, "Tuning optimizer for fine tuning", separator]))
...
def main(argv: Optional[List[str]] = None):
    ...
    setup_logger(file_logger=path.join(args.logdir, config.timestamp, "log.log"))
