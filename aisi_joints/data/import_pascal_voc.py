"""Preprocesses .xml and image files in PASCAL VOC format together with RCM
API.

.csv files with context information. Bounding boxes and labels are extracted
from the .xml files, and matched to image using eventID. merging .csv fields
with labels.
...
from ..constants import CLASS_DEFECT
from ..constants import CLASS_OK
from ..constants import LABEL_MAP
from .common import find_images
from .common import find_labels
from .common import write_pbtxt
...
def import_pascal_voc(
    ...
    log.info(f"Total number of non-defects: " f'{len(df[df["cls"] == CLASS_OK])}.')
    log.info(f"Total number of defects: " f'{len(df[df["cls"] == CLASS_DEFECT])}.')
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output", default="output.csv", help="Output csv name")
