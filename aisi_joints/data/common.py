...
def find_labels(labels_pth: List[str]) -> pd.DataFrame:
    ...
    for label_pth in labels_pth:
        if not path.isfile(label_pth):
            raise FileNotFoundError(f"Could not find label file at " f"{label_pth}.")
    ...
    log.info(f"Registered {len(labels_df)} labels from {len(labels_pth)} " f"files.")
...
def find_images(images_pth: List[str], find_dims: bool = True) -> pd.DataFrame:
    ...
    extracts eventId from the filename and maps the eventId to the image path
    ...
    log.info(
        f"Registered {len(images_idx)} images in {len(images_pth)} " f"directories."
    )
...
class Sample:
    ...
    def from_dataframe(df: pd.DataFrame) -> "Sample":
        ...
        sample.bbox = DetectionBox(df["x0"], df["x1"], df["y0"], df["y1"], df["cls"])
