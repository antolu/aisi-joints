...
def evaluate(df: pd.DataFrame, model: LinearClassifierMethod) -> pd.DataFrame:
    """Calculate predictions from input data Print results to terminal."""
    ...
    log.info(f"Done. Took {t.duration * 1000 / len(labels)} ms " f"per sample.")
...
def main(argv: Optional[List[str]] = None):
    ...
    if os.path.isfile(args.model):
        ...
        raise FileNotFoundError("Could not find checkpoint file at " + args.model)
