...
def train_encoder(
    ...
    if log_dir is not None:
        logger = TensorBoardLogger(path.join(log_dir, f"model-base-{timestamp}"))
    else:
        logger = True
...
def train_classifier(
    ...
    model = LinearClassifierMethod.from_moco_checkpoint(checkpoint_path, **params_dict)
    ...
    if log_dir is not None:
        logger = TensorBoardLogger(path.join(log_dir, f"model-classifier-{timestamp}"))
    else:
        logger = True
...
def train(dataset_path: str, checkpoint_dir: str, log_dir: str, config, mode: str):
    ...
    if mode in ("both", "linear"):
        classifier_params: LinearClassifierMethodParams = config.classifier_params
        # model loading
        if mode == "both":
